...
abstract class AbstractView {
    ...
    public async function render(): Awaitable<XHPRoot> {
        concurrent {
            $title = await \HH\Asio\join($this->getTitleAsync());
            $content = await \HH\Asio\join($this->getContentPaneAsync());
        }
    ...
    final protected async function getContentPaneAsync(): Awaitable<XHPRoot> {
        concurrent {
            $heading = await \HH\Asio\join ($this->getHeadingAsync());
            $body = await \HH\Asio\join($this->getBodyAsync());
        }
